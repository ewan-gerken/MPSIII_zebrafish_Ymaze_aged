...
meta <- read_xlsx("data/metadata/sgsh aged behavioural metadata.xlsx") %>% 
...
final_data <- read_csv("data/processed_data/sgsh/aged/final_output.csv") %>% 
  dplyr::select(-1) %>% 
...
sgshagedgenoproportions <- 
  ...
  labs(title = "***sgsh*** Y-maze genotype proportions",
       y = "Number of fish", 
       x = "")

saveRDS(sgshagedgenoproportions, "data/R objects/supps/sgshagedgenoproportions.rds")
```

#genotype proportions combined plot making

```{r}

sgshagedgenoproportions <- readRDS("data/R objects/supps/sgshagedgenoproportions.rds")
sgshagedgenoproportions <- readRDS("data/R objects/supps/sgshagedgenoproportions.rds")
hgsnatagedgenoproportions <- readRDS("data/R objects/supps/hgsnatagedgenoproportions.rds")

  
ggarrange(sgshagedgenoproportions, sgshagedgenoproportions, hgsnatagedgenoproportions, common.legend = T, legend = "none", nrow = 1)

ggsave(filename = "agedcombinedgenoproportions.svg", path = "output/Publication", width = 12, height = 6, dpi = 300)
...
```{r}
sgshaged_raw_turns <- final_data_summedoverbins %>%
  ungroup() %>%
  group_by(fish_id) %>%
...
#raw turns combined plot making

```{r}
sgshaged_raw_turns <- readRDS("data/R objects/supps/sgshaged_raw_turns.rds")
sgshaged_raw_turns <- readRDS("data/R objects/supps/sgshaged_raw_turns.rds")
hgsnataged_raw_turns <- readRDS("data/R objects/supps/hgsnataged_raw_turns.rds")


  
ggarrange(sgshaged_raw_turns, sgshaged_raw_turns, hgsnataged_raw_turns, common.legend = T, legend = "bottom", nrow = 1)

ggsave(filename = "agedcombined_rawturns.svg", path = "output/Publication", width = 12, height = 6, dpi = 300)
...
#commenting this out as I have saved the glm into an rds file
# glm_timebyturns2sgshaged <- final_data_summedoverbins %>%
#   ungroup() %>%
#   group_by(fish_id) %>%
#   mutate(total_turns = sum(x)) %>%
#   dplyr::select(colnames(meta), total_turns) %>% 
...
saveRDS(glm_timebyturns2sgshaged, "data/R objects/supps/glm_timebyturns2sgshaged.rds")

glm_timebyturns2sgshaged <- readRDS("data/R objects/supps/glm_timebyturns2sgshaged.rds")

Anova(glm_timebyturns2sgshaged) %>% 
  signif(digits = 2) %>% 
  kable %>% 
  kable_styling(full_width = F) %>% 
  row_spec(color = "red", row = c(1, 3))

print(emmeans(sgshagedglmturns_fishid, ~ genotype), type = "response") %>% 
  as_tibble() %>% 
...
sgshaged_turnsacrosshour <- 
  ...
  labs(title = "Number of turns performed by ***sgsh^S387Lfs^*** zebrafish in the Y-maze across 1 hour ", 
       y = "Number of turns", x = "Time bin") +
  scale_colour_viridis_d(end = 0.75) +
  scale_fill_viridis_d(end = 0.75)

saveRDS(sgshaged_turnsacrosshour, "data/R objects/supps/sgshaged_turnsacrosshour.rds")
```

## turns across hour combined plot

```{r}
sgshaged_turnsacrosshour <- readRDS("data/R objects/supps/sgshaged_turnsacrosshour.rds")
sgshaged_turnsacrosshour <- readRDS("data/R objects/supps/sgshaged_turnsacrosshour.rds")
hgsnataged_turnsacrosshour <- readRDS("data/R objects/supps/hgsnataged_turnsacrosshour.rds")


  
ggarrange(sgshaged_turnsacrosshour, sgshaged_turnsacrosshour, hgsnataged_turnsacrosshour, common.legend = T, legend = "bottom", nrow = 3)

ggsave(filename = "agedcombined_turnsacrosshour.svg", path = "output/Publication", width = 12, height = 15, dpi = 300)

```

## turns across hour model predicted combined plot

```{r}
sgshaged_turnsacrosshour_glm_fishid <- readRDS("data/R objects/fish_id/sgshaged_turnsacrosshour_glm_plot_fishid.rds")
nagluaged_turnsacrosshour_glm_fishid <- readRDS("data/R objects/fish_id/nagluaged_turnsacrosshour_glm_plot_fishid.rds")
hgsnataged_turnsacrosshour_glm_fishid <- readRDS("data/R objects/fish_id/hgsnataged_turnsacrosshour_glm_plot_fishid.rds")


  
ggarrange(sgshaged_turnsacrosshour_glm_fishid, nagluaged_turnsacrosshour_glm_fishid, hgsnataged_turnsacrosshour_glm_fishid, common.legend = T, legend = "bottom", nrow = 3)

ggsave(filename = "agedcombined_turnsacrosshour_glm_plot_fishid.svg", path = "output/Publication", width = 12, height = 15, dpi = 300)
...
glm_timebyturnssgshaged_fishid %>% saveRDS("/Users/admin/Desktop/R projects/MPSIII_zebrafish_Ymaze-main/data/R objects/sgsh_aged_totalturns_glm_fishid.rds")

# re-import this object
sgsh_aged_totalturns_glm_fishid <- read_rds("/Users/admin/Desktop/R projects/MPSIII_zebrafish_Ymaze-main/data/R objects/fish_id/sgsh_aged_totalturns_glm_fishid.rds")

#the anova chunk below gives me xml children subscript out of bounds error

#sgshaged_timebyturns_anova <- 
  Anova(sgsh_aged_totalturns_glm_fishid) %>% 
  signif(digits = 2) %>%
  kable %>%
  kable_styling(full_width = F) #%>%
 row_spec(color = "red", row = c(1, 3, 4))

write.csv(sgshaged_timebyturns_anova, "/Users/admin/Desktop/R projects/MPSIII_zebrafish_Ymaze-main/data/R objects/sgshaged_timebyturns_anova.csv", row.names = TRUE)

sgshaged_timebyturns_fishid_anova <- Anova(glm_timebyturnssgshaged) %>% signif(digits = 3)

write.csv(sgshaged_timebyturns_fishid_anova, "/Users/admin/Desktop/R projects/MPSIII_zebrafish_Ymaze-main/data/R objects/sgshaged_timebyturns_fishid_anova.csv", row.names = TRUE)

print(emmeans(glm_timebyturnssgshaged_fishid, ~ genotype * bin), type = "response") %>% 
  as_tibble() %>% 
  mutate(binforvis = case_when(
    bin == 1 ~ "0-10 mins", 
    bin == 2 ~ "10-20 mins", 
...
sgshaged_turnsacrosshour_glm_plot_fishid <- 
 print(emmeans(glm_timebyturnssgshaged_fishid, ~ genotype * bin), type = "response") %>% 
  ...
  labs(title = "Model predicted number of turns performed by ***sgsh^S387Lfs^*** zebrafish in the Y-maze across 1 hour ", 
       subtitle = "Effect of genotype * time bin p = 0.003 ", y = "Number of turns", x = "Time bin") +
...
#effect of sex on total turns glm

print(emmeans(sgsh_aged_totalturns_glm, ~sex), type ="response") %>% 
  as_tibble() %>% 
  ...
  #facet_wrap(~sex, nrow = 1) +
            theme(axis.title.x = element_text(size = 18, margin = margin(b = 5, t = 15))) +
            theme(axis.title.y = element_text(size = 16)) +
            theme(axis.text = element_text(size = 14)) +
  ...
  scale_fill_viridis_d(end = 0.75, begin = 0.25) +
  labs(y = "predicted average turns per bin",
       title = "Activity of *sgsh ^S387Lfs^* zebrafish ", 
       subtitle = "Effect of sex p < 0.001"
       ) +
  scale_x_discrete(labels = c("female", "male"))


#effect of tank on total turns glm

print(emmeans(sgsh_aged_totalturns_glm, ~tank), type ="response") %>% 
  as_tibble() %>% 
  ggplot(aes(x = tank, y = response, colour = tank)) +
  geom_col(aes(fill = tank), 
           alpha = 0.45,
           width = 0.6,
           position = position_dodge()) +
  geom_errorbar(aes(ymin = asymp.LCL, ymax =asymp.UCL),
  ...
  scale_color_viridis_d(begin = 0.01, end = 0.02, option = "inferno") +
  scale_fill_viridis_d(end = 0.75, begin = 0.25) +
  labs(y = "predicted average turns per bin", x = "clutch number",
       title = "Activity of *sgsh ^S387Lfs^* zebrafish ", 
       subtitle = "Effect of clutch p = 0.017"
       )

#ggsave(filename = "sgshagedtotalturnsnewpvalue.svg", path = "output/ZF presentation", width = 12, height = 6, dpi = 300)
...
Can use the below code to define non-responsive fish (under a certain total turn number). KB used 60 as her cutoff. I had a play, there are only wt and hets under 200 turns. The lowest hom is 211.
...
```{r}
read_csv("/Users/admin/Desktop/R projects/MPSIII_zebrafish_Ymaze-main/data/processed_data/sgsh/aged/time_in_zone.csv") %>%
...
read_csv("/Users/admin/Desktop/R projects/MPSIII_zebrafish_Ymaze-main/data/processed_data/sgsh/aged/time_in_zone.csv") %>%
  ...
  ggplot(aes(x = genotype, y =aveTimeInZone/60)) +
  geom_quasirandom(aes(colour = genotype)
                  ) +
  geom_boxplot(aes(fill = genotype),
               outlier.shape = NA,
...
fit <- read_csv("/Users/admin/Desktop/R projects/MPSIII_zebrafish_Ymaze-main/data/processed_data/sgsh/aged/time_in_zone.csv") %>%
  mutate(fish_id = as.character(fish_id)) %>%
  #dplyr::filter(!(fish_id %in% fish2omit)) %>% 
...
```{r}
sgshaged_tetrafreq <- final_data_summedoverbins %>%
  ...
  theme(plot.title = element_markdown(size = 14, hjust = 0.5), plot.subtitle = element_markdown(hjust = 0.5)) +
        labs(title = "Tetragram frequency of ***sgsh^S387Lfs^*** zebrafish in the Y-maze during 1 hour ", y = "tetragram frequency")

 saveRDS(sgshaged_tetrafreq, "data/R objects/supps/sgshaged_tetrafreq.rds")
```

## tetragram frequency aged combined plot making

```{r}
sgshaged_tetrafreq <- readRDS("data/R objects/supps/sgshaged_tetrafreq.rds")
sgshaged_tetrafreq <- readRDS("data/R objects/supps/sgshaged_tetrafreq.rds")
hgsnataged_tetrafreq <- readRDS("data/R objects/supps/hgsnataged_tetrafreq.rds")

  
ggarrange(sgshaged_tetrafreq, sgshaged_tetrafreq, hgsnataged_tetrafreq, common.legend = T, legend = "bottom", nrow = 3)

ggsave(filename = "agedcombined_tetrafreq.svg", path = "output/Publication", width = 12, height = 15, dpi = 300)
...
sgshaged_alts_glm_anova <- Anova(sgshaged_alts_glm) %>% signif(digits = 3)
  #%>%
...
write.csv(sgshaged_alts_glm_anova, "/Users/admin/Desktop/R projects/MPSIII_zebrafish_Ymaze-main/data/R objects/sgshaged_alts_glm_anova.csv", row.names = TRUE)
```
...
```{r}
print(emmeans(sgshaged_alts_glm, ~ genotype), type = "response") %>% 
  as_tibble() %>% 
...
```{r}
print(emmeans(sgshaged_alts_glm, ~ genotype * bin), type = "response") %>% 
  as_tibble() %>% 
  mutate(binforvis = case_when(
    bin == 1 ~ "0-10 mins", 
    bin == 2 ~ "10-20 mins", 
...
sgsh_aged_glm_reps_anova <- Anova(sgsh_aged_glm_reps) %>%
  as.data.frame() 
  #%>%
  # dplyr::rename(pval = `Pr(>Chisq)`) %>%
  # kable() %>%
...
write.csv(sgsh_aged_glm_reps_anova, "/Users/admin/Desktop/R projects/MPSIII_zebrafish_Ymaze-main/data/R objects/sgsh_aged_glm_reps_anova.csv", row.names = TRUE)
```
...
```{r}
print(emmeans(sgshaged_glm_reps, ~ genotype), type = "response") %>% 
  as_tibble() %>% 
  ggplot(aes(x = genotype, y = prob)) +
  geom_col(aes(fill = genotype), 
           alpha = 0.5,
...
```{r, fig.height=10}
print(emmeans(sgshaged_glm_reps, ~ genotype * bin), type = "response") %>% 
  as_tibble() %>% 
  mutate(binforvis = case_when(
    bin == 1 ~ "0-10 mins", 
    bin == 2 ~ "10-20 mins", 
  ...
  labs(y = "Estimated probability of alternation\n(LRLR + RLRL)",
       ...
       subtitle = "Effect of genotype*bin p = 0.12"
       )
```
...
```{r}
sgshagedglm <- readRDS("data/R objects/sgsh_aged_glm.rds")
sgshagedglm <- readRDS("data/R objects/sgsh_aged_glm.rds")
hgsnatagedglm <- readRDS("data/R objects/hgsnat_aged_glm.rds")
...
ggsave(filename = "MPSIII_aged_combined_altsandturns.svg", path = "output/Publication", width = 12, height = 10, dpi = 300)
```

### total turns combined plot making

```{r}
sgshagedglmturns_fishid <- readRDS("data/R objects/fish_id/sgsh_aged_totalturns_glm_fishid.rds")
nagluagedglmturns_fishid <- readRDS("data/R objects/fish_id/naglu_aged_totalturns_glm_fishid.rds")
hgsnatagedglmturns_fishid <- readRDS("data/R objects/fish_id/hgsnat_aged_timebyturns_glm_fishid.rds")
...
genotypeggplotFun <- function(glm, title) {
  print(emmeans(glm, specs = "genotype"), type = "response") %>% 
    ...
    scale_color_manual(values = c("wt" = "#3F33FF",
                                "het" = "orange",
                                "hom" = "steelblue")) +
...
#agedcombturns <- 
  ggarrange(
  genotypeggplotFun(glm = sgshagedglmturns_fishid, title = "*sgsh ^S387Lfs^*  p = 0.023"),
  genotypeggplotFun(glm = nagluagedglmturns_fishid, title = "*naglu ^A603Efs^*  p = 0.001"),
  genotypeggplotFun(glm = hgsnatagedglmturns_fishid, title = "*hgsnat ^G577Sfs^* p = 0.54"),
  #genotypeggplotFun(glm = sgshaged, title = "aged: p = 0.24"),
  common.legend = T,
  legend = "none",
  nrow = 1
)

ggsave(filename = "MPSIII_aged_combinedturns_paper1_fishid.svg", path = "output/Publication", width = 12, height = 6, dpi = 300)
