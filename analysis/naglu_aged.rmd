...
meta <- read_xlsx("data/metadata/naglu aged behavioural metadata.xlsx") %>% 
...
final_data <- read_csv("data/processed_data/naglu/aged/final_output.csv") %>% 
  dplyr::select(-1) %>% 
...
nagluagedgenoproportions <- 
  ...
  labs(title = "***naglu*** Y-maze genotype proportions",
       y = "Number of fish", 
       x = "")

saveRDS(nagluagedgenoproportions, "data/R objects/supps/nagluagedgenoproportions.rds")
```
...
naglu3mgenoproportions <- readRDS("data/R objects/supps/naglu3mgenoproportions.rds")
naglu6mgenoproportions <- readRDS("data/R objects/supps/naglu6mgenoproportions.rds")
naglu9mgenoproportions <- readRDS("data/R objects/supps/naglu9mgenoproportions.rds")

#naglu369combinedgenoproportions <- 
  
ggarrange(naglu3mgenoproportions, naglu6mgenoproportions, naglu9mgenoproportions, common.legend = T, legend = "none", nrow = 1)

ggsave(filename = "naglu369combinedgenoproportions.svg", path = "output/Publication", width = 12, height = 6, dpi = 300)
...
```{r}
nagluaged_raw_turns <- final_data_summedoverbins %>%
  ungroup() %>%
  group_by(fish_id) %>%
  ...
  labs(y = "Total number of turns in 1 hour", 
       title = "Total turns by ***naglu^A603Efs^*** fish")

saveRDS(nagluaged_raw_turns, "data/R objects/supps/nagluaged_raw_turns.rds")
...
glm_timebyturns2 <- final_data_summedoverbins %>%
  ...
  unique  %>%
  ungroup %>% 
  glmer.nb(total_turns ~ genotype + tank + sex + arena_position + zantiks_unit + (1|start_time),
         data = .)
...
#nagluaged_turnsacrosshour <- 
  ...
  labs(title = "Number of turns performed by ***naglu^A603Efs^*** zebrafish in the Y-maze across 1 hour ", 
       y = "Number of turns", x = "Time bin") +
  scale_colour_viridis_d(end = 0.75) +
  scale_fill_viridis_d(end = 0.75)

saveRDS(nagluaged_turnsacrosshour, "data/R objects/supps/nagluaged_turnsacrosshour.rds")
```
...
naglu_aged_totalturns_glm_fishid <- final_data_long %>%
  dplyr::distinct(fish_id, bin, .keep_all = TRUE) %>%
...
naglu_aged_totalturns_glm_fishid %>% saveRDS("/Users/admin/Desktop/R projects/MPSIII_zebrafish_Ymaze-main/data/R objects/naglu_aged_totalturns_glm_fishid.rds")

# re-import this object

naglu_aged_totalturns_glm_fishid <- read_rds("/Users/admin/Desktop/R projects/MPSIII_zebrafish_Ymaze-main/data/R objects/fish_id/naglu_aged_totalturns_glm_fishid.rds")



#nagluaged_timebyturns_anova_fishid <- 
...
write.csv(nagluaged_timebyturns_anova_fishid, "/Users/admin/Desktop/R projects/MPSIII_zebrafish_Ymaze-main/data/R objects/nagluaged_timebyturns_anova_fishid.csv", row.names = TRUE)
...
print(emmeans(glm_timebyturns, ~ genotype * bin), type = "response") %>% 
  ...
  labs(y = "Predicted number of turns per bin",
       x = "Time interval", 
       title = "GLM-predicted total turns per bin in naglu A603Efs", 
       subtitle = "Effect of genotype*bin p = 1. effect of genotype p = "
       )



nagluaged_turnsacrosshour_glm_plot_fishid <- 
 print(emmeans(naglu_aged_totalturns_glm_fishid, ~ genotype * bin), type = "response") %>% 
  as_tibble() %>% 
  mutate(binforvis = case_when(
    bin == 1 ~ "0-10 mins", 
    bin == 2 ~ "10-20 mins", 
  ...
  scale_y_continuous(limits = c(0,205), breaks = seq(0,205,25)) +
  
  labs(title = "Model predicted number of turns performed by ***naglu^A603Efs^*** zebrafish in the Y-maze across 1 hour ", 
       subtitle = "Effect of genotype * time bin p = 0.99 ", y = "Number of turns", x = "Time bin") +
  scale_colour_viridis_d(end = 0.8) +
  scale_fill_viridis_d(end = 0.8)
 
 saveRDS(nagluaged_turnsacrosshour_glm_plot_fishid, "data/R objects/fish_id/nagluaged_turnsacrosshour_glm_plot_fishid.rds")
...
print(emmeans(glm_timebyturns, ~ genotype), type ="response") %>% 
  ...
  labs(y = "predicted average number of turns performed in 1 hour",
       x = "naglu A603Efs", 
       title = "Activity of naglu A603Efs zebrafish in a y-maze ", 
       subtitle = "Effect of genotype p < 0.001"
       )

ggsave(filename = "nagluagedtotalturnsnewpvalue.svg", path = "output/ZF presentation", width = 12, height = 6, dpi = 300)
...
#glm %>% 
  saveRDS("/Users/admin/Desktop/R projects/MPSIII_zebrafish_Ymaze-main/data/R objects/naglu_aged_turns.rds")
```
...
Can use the below code to define non-responsive fish (under a certain total turn number). KB used 60 as her cutoff. I had a play, there are only wt and hets under 400 turns. The naglu fish seemed to do more turns than the sgsh fish at the aged testing.
 
```{r}
final_data %>% 
  dplyr::select(!grep(colnames(final_data), pattern = "[L|R]{4}")) %>% #unsleect the tetras cols to make my life easier right now. 
  group_by(fish_id) %>% 
  mutate(nonresp = sum(total_turns) < 400) %>% 
...
```{r}
read_csv("/Users/admin/Desktop/R projects/MPSIII_zebrafish_Ymaze-main/data/processed_data/naglu/aged/time_in_zone.csv") %>%
  ...
  labs(y = "Total time spent in each zone (mins, log scale)", 
       title = "Total time spent in each zone naglu A603Efs")
```
...
read_csv("/Users/admin/Desktop/R projects/MPSIII_zebrafish_Ymaze-main/data/processed_data/naglu/aged/time_in_zone.csv") %>%
  ...
  easy_rotate_x_labels(angle = -45) +
  labs(y = "Total time spent in each zone (mins, log scale)", 
       title = "Total time spent in each zone for naglu A603Efs", 
       subtitle = "accoring to position in the ymaze")
```

I also wanted to look at the average time spent in each zone. 
...
read_csv("/Users/admin/Desktop/R projects/MPSIII_zebrafish_Ymaze-main/data/processed_data/naglu/aged/time_in_zone.csv") %>%
  ...
  labs(y = "Average time spent in each zone (mins, log scale)", 
       title = "Average time spent in each zone naglu A603Efs")
...
fit <- read_csv("/Users/admin/Desktop/R projects/MPSIII_zebrafish_Ymaze-main/data/processed_data/naglu/aged/time_in_zone.csv") %>%
  mutate(fish_id = as.character(fish_id)) %>%
  #dplyr::filter(!(fish_id %in% fish2omit)) %>% 
...
final_data %>%
  ...
  geom_mark_ellipse(aes(fill = L_R_bias, label = L_R_bias),
                 alpha = 0.2, 
                 con.cap = 0) +
  labs(title = " naglu A603Efs By genotype") +
...
The overall propotions of fish showing left, right or no bias is simialar across the 4 experimental groups. 
```{r}
final_data %>%
  left_join(LR_Bias) %>%
  group_by(fish_id) %>%
...
nagluaged_tetrafreq <- 
  ...
  theme(plot.title = element_markdown(size = 14, hjust = 0.5), plot.subtitle = element_markdown(hjust = 0.5)) +
        labs(title = "Tetragram frequency of ***naglu^A603Efs^*** zebrafish in the Y-maze during 1 hour ", y = "tetragram frequency")

 saveRDS(nagluaged_tetrafreq, "data/R objects/supps/nagluaged_tetrafreq.rds")
```
...
final_data_summedoverbins %>%
  ...
  annotate("rect", 
           ...
           ymin = -1, ymax = 200,
           alpha = 0, color= "red") +
  ggtitle("Total number of 16 possible tetragrams performed by naglu zebrafish in a Y-maze\nduring a 1 hour search period") 
```
...
nagluaged_alts_glm_anova <- Anova(nagluaged_alts_glm) %>%
  as.data.frame() 
  #%>%
...
write.csv(nagluaged_alts_glm_anova, "/Users/admin/Desktop/R projects/MPSIII_zebrafish_Ymaze-main/data/R objects/nagluaged_alts_glm_anova.csv", row.names = TRUE)
```
...
```{r}
naglu_aged_glm_reps <- final_data %>%
  left_join(LR_Bias) %>%
  mutate(
...
naglu_aged_glm_reps_anova <- Anova(naglu_aged_glm_reps) %>%
  as.data.frame() 
  #%>%
  # dplyr::rename(pval = `Pr(>Chisq)`) %>%
  # kable() %>%
...
write.csv(naglu_aged_glm_reps_anova, "/Users/admin/Desktop/R projects/MPSIII_zebrafish_Ymaze-main/data/R objects/naglu_aged_glm_reps_anova.csv", row.names = TRUE)
```
...
print(emmeans(glm_reps, ~ genotype), type = "response") %>% 
  ...
  labs(y = "Estimated probability of repetitions\n(LLLL + RRRR)",
       ...
       subtitle = "effect of genotype p = 0.26"
       )
```
...
print(emmeans(glm_reps, ~ genotype * bin), type = "response") %>% 
  ...
  labs(y = "Estimated probability of alternation\n(LRLR + RLRL)",
       ...
       subtitle = "Effect of genotype*bin p = 0.35"
       )
```
...
final_data %>% 
  saveRDS("/Users/admin/Desktop/R projects/MPSIII_zebrafish_Ymaze-main/data/R objects/naglu_aged.rds")
```

```{r}
# export glmTMB for combined alternation plot:
glm %>% 
  saveRDS("/Users/admin/Desktop/R projects/MPSIII_zebrafish_Ymaze-main/data/R objects/naglu_aged_glm.rds")
```
